	...
	def before_session_starts(self):
		...
		for p in self.get_players():
			...
			if self.round_number == 1:
				p.period_number = 1
			elif (((self.round_number - 1) / self.session.config['numSubperiods']) % 1 != 0):
				p.period_number = p.in_round(self.round_number - 1).period_number
			else: p.period_number = p.in_round(self.round_number - 1).period_number + 1


			if p.period_number <= len(self.session.config['t']):
				periodIndex = (p.period_number - 1)
				subperiodIndex = (p.period_number % len(self.session.config['t'][periodIndex]))
				p.transport_cost = self.session.config['t'][periodIndex][subperiodIndex]


			if p.period_number <= len(self.session.config['mc']):
				periodIndex = (p.period_number - 1)
				subperiodIndex = (p.period_number % len(self.session.config['mc'][periodIndex]))
				p.mc = self.session.config['mc'][periodIndex][subperiodIndex]


			if p.period_number <= len(self.session.config['rp']):
				periodIndex = (p.period_number - 1)
				subperiodIndex = (p.period_number % len(self.session.config['rp'][periodIndex]))
				p.rp = self.session.config['rp'][periodIndex][subperiodIndex]
